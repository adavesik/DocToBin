...
<div class="d-flex" id="wrapper">
    ...
    <div id="page-content-wrapper">
        ...
        <div class="py-5 text-center">
            <h2 style="color: #007bff">Pads Generating with CRS </h2>
            <p class="lead">Below is an example form built for generating Pads with crs.</p>
        </div>
        <div class="row ml-2" id="binary">
            <div class="col-md-8 order-md-1">
                <form enctype="multipart/form-data" id="userKeyForm">
                    <div class="form-group">
                        <label for="file">File</label>
                        <input type="file" class="form-control" id="file" name="file" required />
                    </div>
                    <select class="custom-select" id="padCount" name="padCount">
                        <option value="" selected>Please select Pads counts to be generated:</option>
                        <option value=1> 1 Pad </option>
                        <option value=2> 2 Pad </option>
                        <option value=3> 3 Pad </option>
                        <option value=4> 4 Pad </option>
                        <option value=5> 5 Pad </option>
                        <option value=6> 6 Pad</option>
                        <option value=7> 7 Pad </option>
                        <option value=8> 8 Pad </option>
                        <option value=9> 9 Pad </option>
                        <option value=10> 10 Pad </option>
                    </select>
                    <hr class="mb-4">
...
<script>
    $(document).ready(function(e){
        $("#userKeyForm").on('submit', function(e){
            e.preventDefault();
            $.ajax({
                type: 'POST',
                url: 'action_userkey.php',
...
<script>
    $(document).ready(function(event){
        $("#userKeyForm").on('submit', function(event){
...
/*            var key = $('#key').val();
            ...
            //generating too many Pads at the same time
            var padCount = $("select#padCount").val();
            if (padCount > 1){
                $.ajax({
                    type: 'POST',
                    url: 'action_crs.php',
